...
func (h *Handler) ListHumanResources(c *gin.Context) {
    ...
    if roleStatus != "" { add("role_status=", roleStatus) }
    if roleType != "" { add("role_type=", roleType) }
    ...
    if err := rows.Err(); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
...
func (h *Handler) CreateHumanResource(c *gin.Context) {
    ...
    for k, v := range requiredStr { if strings.TrimSpace(v) == "" { c.JSON(http.StatusBadRequest, gin.H{"error": k+" is required"}); return } }
    if in.HeadcountNeed <= 0 { c.JSON(http.StatusBadRequest, gin.H{"error": "headcount_need must be > 0"}); return }
    if in.HeadcountGot < 0 { c.JSON(http.StatusBadRequest, gin.H{"error": "headcount_got must be >= 0"}); return }

    id := "hr-" + uuid.New().String()
    // Convert epoch timestamps to *time.Time
    toTime := func(ts *int64) *time.Time { if ts == nil { return nil }; t := time.Unix(*ts, 0).UTC(); return &t }
...
func (h *Handler) PatchHumanResource(c *gin.Context) {
    ...
    args := []interface{}{}
    idx := 1
    add := func(expr string, v interface{}) { setParts = append(setParts, expr+"$"+strconv.Itoa(idx)); args = append(args, v); idx++ }
    // Simple fields
    if in.Org != nil { add("org=", *in.Org) }
    if in.Address != nil { add("address=", *in.Address) }
    if in.Phone != nil { add("phone=", *in.Phone) }
    if in.Status != nil { add("status=", *in.Status) }
    if in.IsCompleted != nil { add("is_completed=", *in.IsCompleted) }
    if in.HasMedical != nil { add("has_medical=", *in.HasMedical) }
    if in.RoleName != nil { add("role_name=", *in.RoleName) }
    if in.RoleType != nil { add("role_type=", *in.RoleType) }
    if in.Skills != nil { add("skills=", sliceOrNil(in.Skills)) }
    if in.Certifications != nil { add("certifications=", sliceOrNil(in.Certifications)) }
    if in.ExperienceLevel != nil { add("experience_level=", *in.ExperienceLevel) }
    if in.LanguageRequirements != nil { add("language_requirements=", sliceOrNil(in.LanguageRequirements)) }
    if in.HeadcountNeed != nil { add("headcount_need=", *in.HeadcountNeed) }
    if in.HeadcountGot != nil { add("headcount_got=", *in.HeadcountGot) }
    if in.HeadcountUnit != nil { add("headcount_unit=", *in.HeadcountUnit) }
    if in.RoleStatus != nil { add("role_status=", *in.RoleStatus) }
    // Time fields (convert epoch to timestamptz)
    toTime := func(ts *int64) *time.Time { if ts == nil { return nil }; t := time.Unix(*ts,0).UTC(); return &t }
    if in.ShiftStartTs != nil { add("shift_start_ts=", toTime(in.ShiftStartTs)) }
    if in.ShiftEndTs != nil { add("shift_end_ts=", toTime(in.ShiftEndTs)) }
    if in.ShiftNotes != nil { add("shift_notes=", *in.ShiftNotes) }
    if in.AssignmentTimestamp != nil { add("assignment_timestamp=", toTime(in.AssignmentTimestamp)) }
    if in.AssignmentCount != nil { add("assignment_count=", *in.AssignmentCount) }
    if in.AssignmentNotes != nil { add("assignment_notes=", *in.AssignmentNotes) }
    if in.TotalRolesInRequest != nil { add("total_roles_in_request=", *in.TotalRolesInRequest) }
    if in.CompletedRolesInRequest != nil { add("completed_roles_in_request=", *in.CompletedRolesInRequest) }
    if in.PendingRolesInRequest != nil { add("pending_roles_in_request=", *in.PendingRolesInRequest) }
    if in.TotalRequests != nil { add("total_requests=", *in.TotalRequests) }
    if in.ActiveRequests != nil { add("active_requests=", *in.ActiveRequests) }
    if in.CompletedRequests != nil { add("completed_requests=", *in.CompletedRequests) }
    if in.CancelledRequests != nil { add("cancelled_requests=", *in.CancelledRequests) }
    if in.TotalRoles != nil { add("total_roles=", *in.TotalRoles) }
    if in.CompletedRoles != nil { add("completed_roles=", *in.CompletedRoles) }
    if in.PendingRoles != nil { add("pending_roles=", *in.PendingRoles) }
    if in.UrgentRequests != nil { add("urgent_requests=", *in.UrgentRequests) }
    if in.MedicalRequests != nil { add("medical_requests=", *in.MedicalRequests) }
...
func sliceOrNil(s []string) interface{} { if s == nil { return nil }; return s }
