...
func (h *Handler) CreateWaterRefillStation(c *gin.Context) {
    ...
    isFree := false; if in.IsFree != nil { isFree = *in.IsFree }
    accessible := false; if in.Accessibility != nil { accessible = *in.Accessibility }
    var lat, lng *float64
    if in.Coordinates != nil { lat = in.Coordinates.Lat; lng = in.Coordinates.Lng }
    ...
    if lat != nil || lng != nil { out.Coordinates = &struct { Lat *float64 `json:"lat"`; Lng *float64 `json:"lng"` }{Lat: lat, Lng: lng} }
...
func (h *Handler) PatchWaterRefillStation(c *gin.Context) {
    ...
    if in.WaterType != nil { add("water_type=", *in.WaterType) }
    if in.OpeningHours != nil { add("opening_hours=", *in.OpeningHours) }
    if in.IsFree != nil { add("is_free=", *in.IsFree) }
    if in.ContainerRequired != nil { add("container_required=", *in.ContainerRequired) }
    if in.DailyCapacity != nil { add("daily_capacity=", *in.DailyCapacity) }
    if in.Status != nil { add("status=", *in.Status) }
    if in.WaterQuality != nil { add("water_quality=", *in.WaterQuality) }
    if in.Facilities != nil { add("facilities=", *in.Facilities) }
    if in.Accessibility != nil { add("accessibility=", *in.Accessibility) }
    if in.DistanceToDisasterArea != nil { add("distance_to_disaster_area=", *in.DistanceToDisasterArea) }
    if in.Notes != nil { add("notes=", *in.Notes) }
    if in.InfoSource != nil { add("info_source=", *in.InfoSource) }
    if in.Coordinates != nil { if in.Coordinates.Lat != nil { add("lat=", *in.Coordinates.Lat) }; if in.Coordinates.Lng != nil { add("lng=", *in.Coordinates.Lng) } }
    ...
    w.Phone = phone; w.ContainerRequired = containerReq; w.WaterQuality = waterQuality; w.DistanceToDisasterArea = distance; w.Notes = notes; w.InfoSource = infoSource; w.DailyCapacity = dailyCap; w.Facilities = facilities; w.IsFree = isFree; w.Accessibility = accessibility; w.CreatedAt = created; w.UpdatedAt = updated
    if lat != nil || lng != nil { w.Coordinates = &struct { Lat *float64 `json:"lat"`; Lng *float64 `json:"lng"` }{Lat: lat, Lng: lng} }
...
func (h *Handler) ListWaterRefillStations(c *gin.Context) {
    ...
    filters := []string{}
    args := []interface{}{}
    if status != "" { filters = append(filters, "status=$"+strconv.Itoa(len(args)+1)); args = append(args, status) }
    if waterType != "" { filters = append(filters, "water_type=$"+strconv.Itoa(len(args)+1)); args = append(args, waterType) }
    if isFree != "" { filters = append(filters, "is_free=$"+strconv.Itoa(len(args)+1)); val := (isFree == "true" || isFree == "1"); args = append(args, val) }
    if accessibility != "" { filters = append(filters, "accessibility=$"+strconv.Itoa(len(args)+1)); val := (accessibility == "true" || accessibility == "1"); args = append(args, val) }
    ...
    var total int
    if err := h.pool.QueryRow(ctx, countQ, args...).Scan(&total); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
    ...
    for rows.Next() {
        ...
        var lat, lng *float64
        var created, updated int64
        if err := rows.Scan(&w.ID, &w.Name, &w.Address, &phone, &w.WaterType, &w.OpeningHours, &free, &containerReq, &dailyCap, &w.Status, &waterQuality, &facilities, &acc, &distance, &notes, &infoSource, &lat, &lng, &created, &updated); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
        w.Phone = phone; w.ContainerRequired = containerReq; w.WaterQuality = waterQuality; w.DistanceToDisasterArea = distance; w.Notes = notes; w.InfoSource = infoSource; w.DailyCapacity = dailyCap; w.Facilities = facilities; w.IsFree = free; w.Accessibility = acc; w.CreatedAt = created; w.UpdatedAt = updated
        if lat != nil || lng != nil { w.Coordinates = &struct { Lat *float64 `json:"lat"`; Lng *float64 `json:"lng"` }{Lat: lat, Lng: lng} }
        list = append(list, w)
    }
